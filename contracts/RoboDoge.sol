/*
   https://bscscan.com/address/0x2f657932e65905ea09c7aacfe898bf79e207c1c0
...
contract RoboDogeCoin is Context, IERC20, Ownable {
    ...
    function excludeAccount(address account) public onlyOwner {
        require(
            ...
            "ROBODOGE: Uniswap router cannot be excluded."
        );
        require(
            account != address(this),
            "ROBODOGE: The contract it self cannot be excluded"
        );
    ...
    function _transfer(
        ...
        if (isLiquidityPoolAddress[recipient] || forceEop) {
            (
                bool isAntiDumping,
                string memory errorMessage
            ) = enforceAntiDumping(sender, amount);
    ...
    function enforceAntiDumping(address sender, uint256 amount)
        private
        returns (bool, string memory)
    {
        ...
        if (balanceOf(sender) >= threshold && sender != owner()) {
            if (
                block.timestamp > userInfo.firstSellTime + restrictionDuration
            ) {
    ...
    function delegateBySig(
        ...
        require(
            signatory != address(0),
            "ROBODOGE::delegateBySig: invalid signature"
        );
        require(
            nonce == nonces[signatory]++,
            "ROBODOGE::delegateBySig: invalid nonce"
        );
        require(
            block.timestamp <= expiry,
            "ROBODOGE::delegateBySig: signature expired"
        );
        ...
        returns (uint256)
    {
        require(
            ...
            "ROBODOGE::getPriorVotes: not yet determined"
        );
    ...
    function _writeCheckpoint(
        ...
        uint32 blockNumber = safe32(
            block.number,
            "ROBODOGE::_writeCheckpoint: block number exceeds 32 bits"
        );
